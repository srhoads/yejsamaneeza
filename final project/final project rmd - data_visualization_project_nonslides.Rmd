...
# We want to explore where Columbia students come from! But first, let's look at US schools and their diversity trends on the aggregate...
...
# NAs goodbye
pub10_url <- url("https://github.com/srhoads/schooldata/blob/master/publicschools/ccd%20raw%20data/sc102a.2010.11.rda?raw=true")
pub2010_11 <- get(load(pub10_url))

pub2010_11 <- subset(pub2010_11, !is.na(pub2010_11$SCHNAM))
pub2010_11 <- subset(pub2010_11, (pub2010_11$TOTFRL >= 0))
pub2010_11 <- subset(pub2010_11, !is.na(pub2010_11$WHITE))
pub2010_11$perc_racial_minorities <- 1 - pub2010_11$WHITE / pub2010_11$MEMBER
pub2010_11$WHITE[pub2010_11$WHITE < 0] <- NA
summary(pub2010_11$perc_white <- ifelse(pub2010_11$WHITE <= pub2010_11$MEMBER, pub2010_11$WHITE / pub2010_11$MEMBER, NA))
pub2010_11$perc_racial_minority <- 1 - pub2010_11$perc_white

pub2010_11 <- pub2010_11 %>% 
  mutate(minority_distrib = (ifelse(perc_racial_minority < .25, "0 to 25% minority", 
                      ifelse(perc_racial_minority >= .25 & perc_racial_minority < .5, "25 - 50% minority",
                             ifelse(perc_racial_minority >= .5 & perc_racial_minority < .75, "50 to 75% minority",
                                    ifelse(perc_racial_minority >= .75 & perc_racial_minority <= 1, "75 to 100% minority", NA))))))

# pub2010_11$minority_distrib <-  factor(pub2010_11$perc_racial_minority, levels = c(0:.24999, .25:.4999, .5:.74999, .75:1), labels = c("Under 1/4 minority", "25% to half minority", "half to 75% minority", "3/4 to 100% minority"))

# pub2010_11 <- pub2010_11[order(-pub2010_11$perc_racial_minorities),] 
summary(pub2010_11$minority_distrib)
```
...
library(leaflet)
pub2010_11_nona <- pub2010_11 %>% filter(!is.na(minority_distrib))

pub2010_11_hs <- pub2010_11_nona %>% filter(G09OFFRD == 1, G10OFFRD == 1, G11OFFRD == 1, G12OFFRD == 1)

# Set up color palette
palette.2 = colorNumeric("Paired", domain = pub2010_11_hs$perc_racial_minority)
race.color = palette.2(pub2010_11_hs$perc_racial_minority)
pub2010_11_hs$TOTFRL
# Set up popup stuffs
stuff.2 <- paste("City:",pub2010_11_hs$MCITY,"<br/>",
                  "State:",pub2010_11_hs$MSTATE,"<br/>",
                  "Total Students:",pub2010_11_hs$MEMBER,"<br/>",
                  "Prop. Minority:",pub2010_11_hs$perc_racial_minority,"<br/>",
                  "Subsidized Lunch:",pub2010_11_hs$TOTFRL,"<br/>")

vis2 <- leaflet(pub2010_11_hs %>% filter(!is.na(minority_distrib))) %>%
  addTiles() %>%
  setView(lng=-100.87, lat=40.78, zoom=4) %>%
  addProviderTiles(providers$Thunderforest) %>%
  addLegend(pal = palette.2, values = ~ pub2010_11_hs$perc_racial_minority, title = "Public High School 2010-2011 Racial Minority Proportion of Total Students (0 to 1)")
...
```{r leaflet map pub schools, echo = F, warning = F, message = F}
vis2 %>%  
  addCircles(lng=~LONCOD, lat=~LATCOD, color = race.color, popup = stuff.2) %>%
...
library(tidyverse)
priv11 <- url("https://github.com/srhoads/schooldata/blob/master/privateschools/pss%20raw%20data/pss1112_pu.rda?raw=true")
pr11 <- data.frame(get(load(priv11)))



pr11 <- subset(pr11, !is.na(pr11$pinst))
pr11 <- subset(pr11, (pr11$numstuds >= 0))
pr11 <- subset(pr11, !is.na(pr11$p_white))
pr11$p_white[pr11$p_white < 0] <- NA
summary(pr11$perc_white <- ifelse(pr11$p_white <= pr11$numstuds, pr11$p_white / pr11$numstuds, NA))
pr11$perc_racial_minority <- 1 - pr11$perc_white
...
summary(priv2011_12$minority_distrib)
# summary(hi <- priv2011_12 %>% select(everything()) %>%
   #       separate(typology, into = c("religion.type", "shit"), sep = " ("))
...
priv2011_12_nona <- priv2011_12 %>% filter(!is.na(minority_distrib))

priv2011_12_hs <- priv2011_12_nona %>% filter(higr2012 == "Highest grade in school is 12th grade" | higr2012 == "Highest grade in school is 11th grade" | higr2012 == "Highest grade in school is 10th grade" | higr2012 == "Highest grade in school is 9th grade")

# Set up color palette
palette.2 = colorNumeric("Paired", domain = priv2011_12_hs$perc_racial_minority)
race.color = palette.2(priv2011_12_hs$perc_racial_minority)
# Set up popup stuffs
stuff.2 <- paste("City:",priv2011_12_hs$pcity,"<br/>",
                  "State:",priv2011_12_hs$pstabb,"<br/>",
                  "Region/Locale:", priv2011_12_hs$ulocale12, "in the", priv2011_12_hs$region,"<br/>",
                  "Magnet?",priv2011_12_hs$typology,"<br/>",
                  "Level",priv2011_12_hs$level,"<br/>")

vis2 <- leaflet(priv2011_12_hs %>% filter(!is.na(minority_distrib))) %>%
  addTiles() %>%
  setView(lng=-100.87, lat=39, zoom =4) %>%
  addProviderTiles(providers$Thunderforest) %>%
  addLegend(pal = palette.2, values = ~ priv2011_12_hs$perc_racial_minority, title = "Prive School 2011-2012 Racial Minority Proportion of Total Students (0 to 1)")
```
                        

                                     
```{r leaflet map priv schools, echo = F, warning = F, message = F}
vis2 %>%  
  addCircles(lng=~longitude12, lat=~latitude12, color = race.color, popup = stuff.2) %>%
  addProviderTiles(providers$CartoDB.Positron)
```
...
## This map includes markers for ALL of the high schools in the US-- public and private!

```{r, echo = F, warning = F, message = F, include = F}
pr <- priv2011_12_hs %>% select(name = pinst, perc_racial_minority, 
                                long = longitude12, lat = latitude12,
                                state = pstabb)
pu <- pub2010_11_hs %>% select(name = SCHNAM, perc_racial_minority,
                               long = LONCOD, lat = LATCOD, 
                               state = MSTATE)

all <- bind_rows(pu, pr)


palette.2 = colorNumeric("Paired", domain = all$perc_racial_minority)
race.color = palette.2(all$perc_racial_minority)
# Set up popup stuffs
stuff.2 <- paste("School Name:",all$name,"<br/>",
                 "State:", all$state, "<br/>",
                  "Prop. Racial Minority:",all$perc_racial_minority,"<br/>")

vis2 <- leaflet(all %>% filter(!is.na(perc_racial_minority))) %>%
  addTiles() %>%
  setView(lng=-100.87, lat=39, zoom =4) %>%
  addProviderTiles(providers$Thunderforest) %>%
  addLegend(pal = palette.2, values = ~ all$perc_racial_minority, title = "High School 2010 Racial Minority Proportion of Total Students (0 to 1)")
```

<br>

```{r, echo = F, warning = F, message = F}
(usa.schools.vis <- vis2 %>%  
  addCircles(lng=~long, lat=~lat, color = race.color, popup = stuff.2) %>%
  addProviderTiles(providers$CartoDB.Positron))
```
...
# write.csv(cul.04, "cul2016.05052018.csv")
cul.05 <- read.csv("https://raw.githubusercontent.com/srhoads/thesis/culties/.gitignore/cul/processed/data/attributesmatched/cul2016_05052018.csv?token=AZbwLofG9NQdq-lqX-NYsubUGlJGuDkNks5a9wqNwA%3D%3D")
...
fire.icon <- makeIcon(iconUrl = "http://cdn.onlinewebfonts.com/svg/img_166278.png", iconWidth = 28, iconHeight = 28)
```
...
usa.schools.cul2016 <- usa.schools.vis %>% addMarkers(lng = cul.04_nona$nces.longitude, lat= cul.04_nona$nces.latitude, 
             popup = ~ paste(as.character(cul.04_nona$hstype), "School in", as.character(cul.04_nona$county.state)), # popup and label as the incident ID to identify outlier non-NYC miscoded units
             label = ~ as.character(paste("Census Tract Diversity:", cul.04_nona$tract.diversity)), 
             icon = fire.icon)
```
...
# Let's gear back to diversity ideologies: colorblindness & multiculturalism
